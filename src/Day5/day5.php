...
class Garden
{
    ...
    {
        $seedData = [$seed, $range];
        $soil = $this->getMappedValue($seedData, $this->seed_to_soil);
    ...
    private function getMappedValue(array &$source, array &$array, array &$result = []): array
    {
        ...
        $range = $source[1];
        $sourceStartMax = $sourceStart + $range;
        ...
        foreach ($indexes as $index) {
            if ($sourceStart >= $index) {
                ...
                $newRange = ($sourceStartMax > $max) ? ($sourceStartMax - $max) : $range;
                if (empty($result)) {
                ...
                if ($sourceStartMax > $max) {
                    ...
                    if ($max <= $highestIndex) {
                        $newSourceStart = $max;
                        $value = [$newSourceStart, $remainder];
                        return ($this->getMappedValue($value, $array, $result));
                    }
                }
